...
from thought.core import Metadata
from thought.exceptions import ServiceNotRegisteredException
from thought.settings import SERVICES_CONFIGURATION_PATH, SERVICES_REGISTERED
...
class GenericService:
    ...
    def __getitem__(self, key) -> Any:
        public = [x for x in self.__dir__() if not (x.startswith('_') or x.endswith('_'))]
    ...
    def load(self):
        """
        Loads service data to notion environment at collection defined in NOTION_SERVICES_DIRECTORY settings variable.
        """
        pass
...
class APIService(GenericService):
    ...
    #     """
    #     Downloads target API's data. If successful the `data` property should contain a pandas dataframe.
...
class Registry:
    ...
    def register(self, service_name: str, return_service_obj: bool = True):
        ...
        if service_name not in SERVICES_REGISTERED.keys():
            raise ServiceNotRegisteredException(f"{service_name} is not a properly configured service")
